...
# copied from iterative_policy_evaluation (in function form)
def get_transition_probs_and_rewards(grid):
    ### define transition probabilities and grid ###
    # the key is (s, a, s'), the value is the probability, that is, transition_probs[(s, a, s')] = p(s' | s, a)
...
def evaluate_deterministic_policy(grid, policy):
    """ We are doing policy improvement, so we're doing argmax of value function and therefore is deterministic.
    """
    # initialize V(s) = 0
    V = {}
...
if __name__ == '__main__':
    ...
    # We can also check without penalty with
    # grid = windy_grid()
